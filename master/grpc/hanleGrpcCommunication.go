	...
	for scanner.Scan() {
		...
		case types.MapInsert:
			...
			if err := json.Unmarshal(message.Input, &insertMapData); err != nil {
				errMessage := types.GenerateErrorMessage(err, message.RequestId)
				conn.Write(append(errMessage, '\n'))
			} else {
				...
				if err != nil {
					...
					jsonMessage, _ = sjson.SetBytes(jsonMessage, "requestId", message.RequestId)
